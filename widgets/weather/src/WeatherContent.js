...
const API_KEY = '66f730fb317d84c5f47a74415868c431'
...
class WeatherContent extends Component {
  ...
  componentDidMount() {
    const { data: { unit = DEFAULT_UNIT, city = DEFAULT_CITY } = {} } = this.props
    city.replace(' ', '+')
    axios
      .get(`${API_URL}/weather?q=${city}&apiKey=${API_KEY}&units=${unit}`)
      .then(({ data }) => {
        const {
          name,
          weather,
        ...
        <style jsx>
          {`
            ...
            .info {
              ...
              justify-content: flex-end;#9900CC
            }
            .icon {
              display: flex;
              flex-direction: column;
