...
class FranqueadosCreate extends Component {
    constructor(props) {
        super(props)
        const {
            _id
    ...
    close = e => {
        ...
        return Promise.resolve().then(
            () => this.dialog && this.dialog.current && this.dialog.current.close() && this.limpaState()
        )
    }
    ...
    saveData = (id) => {
        ...
        return !id ? criaFranqueado(model).then((res) => {
        ...
        }) : atualizaFranqueado(id, model).then((res) => {
            if (res.success == false) {
                alerta(res.msg, 'erro')
            } else {
                alerta(res.msg, '')
    ...
    render() {
        ...
        const { _id, name, email, username, repeatSenha, password, ativo, espaco, licencas, valorMb, valorLicenca } = this.state
        return (
            <div className={styles.modalContainer}>
                <Dialog ref={this.dialog}>
                    <Form>
                        <h3>Criar novo franqueado</h3>
                    ...
                    <ButtonGroup style={{ marginTop: 20 }}>
                        ...
                        />
                        <Button text={'Save'} color={'#9900CC'} onClick={() => this.saveData(_id)} />
...
export default FranqueadosCreate
