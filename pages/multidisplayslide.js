...
import SlideList from '../components/Admin/MultiDisplay/SlideList'
import MultiDisplayEditDialog from '../components/Admin/MultiDisplay/MultiDisplayEditDialog'
...
import InlineInputGroup from '../components/Form/InlineInputGroup.js'
...
class MultidisplaySlide extends React.Component {
  constructor(props) {
    super(props)
    const { slideshow } = props
    this.state = { slideshow }
    this.slideList = React.createRef()
    this.dialogMulti = React.createRef()
  }
  ...
  handleChange = (name, value) => {
    this.setState({
      ...
      () => {
        updateSlideshowThrottled(this.state.slideshow._id, { [name]: value })
      })
  }
  ...
  openAddDialog = () => {
    console.log('aqui', this.dialogMulti)
    return Promise.resolve(this.dialogMulti && this.dialogMulti.current.open())
  }

  onChecked = (id, checked) => {
    let isChecked = checked;
    return updateSlideshowThrottled(id, { showProgress: checked })
  }
  ...
  render() {
    ...
    return (
      <Frame loggedIn={loggedIn}>
        <Head>
          <title>Etag Digital - {slideshow.title || "Slideshow"}</title>
        </Head>
        ...
        <div className='wrapper'>
          ...
          <MultiDisplayEditDialog
            slideshow={slideshow && slideshow._id}
            refresh={this.refresh}
            ref={this.dialogMulti}
          />
          <Button
            text='Adicionar um display'
            icon={'tv'}
            color='#9900CC'
          ...
          <div className='optionsContainer'>
            <h1>Opções do Slideshow</h1>
            <div className='optionsContent'>

              <p className='tempoP'>Ativar barra de tempo?</p>
              <input type={'checkbox'}
                size={25}
                value={slideshow.showProgress}
                checked={slideshow.showProgress}
                onChange={() => {
                  const showProgress = !slideshow.showProgress
                  this.setState(
                    {
                      slideshow: {
                        ...slideshow,
                    ...
                    () => {
                      updateSlideshowThrottled(slideshow._id, { showProgress })
                    }
                  )
                }}
              />
...
export default protect(view(MultidisplaySlide))
